...
test.describe('Inicialización', () => {
    ...
    test('muestra los 3 tabs: Todos, Favoritos, Recientes', async ({
        page,
    }) => {
        await waitForApp(page);
    ...
    test('tab "Todos" está activo por defecto', async ({ page }) => {
        await waitForApp(page);
        await expect(
            page.locator('[data-tab="all"][data-active="true"]'),
        ).toBeVisible();
    });
    ...
    test('AsyncStats se carga y muestra estadísticas', async ({ page }) => {
        ...
        await expect(page.locator('[data-async-stats="true"]')).toBeVisible({
            timeout: 5000,
        });
    });
});
...
test.describe('Validación Zod (TypeScript genéricos complejos)', () => {
    ...
    test('form vacío muestra errores en nombre y email', async ({ page }) => {
        await page
            .locator('[data-contact-form="true"] button[type="submit"]')
            .click();
    ...
    test('datos válidos no muestran errores', async ({ page }) => {
        ...
        await expect(
            page.locator('[data-field-error="name"]'),
        ).not.toBeVisible();
        await expect(
            page.locator('[data-field-error="email"]'),
        ).not.toBeVisible();
    });
});
...
test.describe('CRUD + localStorage', () => {
    test('agregar contacto lo muestra en la lista', async ({ page }) => {
        ...
        await expect(page.locator('[data-contact-list="true"]')).toContainText(
            'Carlos López',
        );
    });
    ...
    test('eliminar contacto lo quita de la lista y del localStorage', async ({
        page,
    }) => {
    ...
    test('toggle favorito cambia el ícono y actualiza localStorage', async ({
        page,
    }) => {
        const contact = makeContact({
            name: 'Toggle Test',
...
test.describe('Persistencia localStorage', () => {
    ...
    test('JSON inválido en localStorage activa ErrorBoundary', async ({
        page,
    }) => {
        ...
        await page.evaluate(
            ({ key }) =>
                localStorage.setItem(
                    key,
                    '{"esto es": "json roto, sin cerrar array',
                ),
            { key: LS_KEY },
        );
        await page.goto(URL);
        ...
        await expect(errorBoundary.or(errorDiv)).toBeVisible({
            timeout: 10000,
        });
    });

    test('array vacío en localStorage muestra estado vacío', async ({
        page,
    }) => {
...
test.describe('Modal via Teleport', () => {
    ...
    test('el modal renderiza dentro de #modal-target (Teleport)', async ({
        page,
    }) => {
        await waitForApp(page);
        await page.locator('[data-action="add-contact"]').click();
    ...
    test('cancelar cierra el modal sin guardar', async ({ page }) => {
        ...
        await page
            .locator('[data-contact-form="true"] button[type="button"]')
            .first()
            .click();

        await expect(page.locator('[data-modal="true"]')).not.toBeVisible();
        await expect(
            page.locator('[data-contact-list="true"]'),
        ).not.toContainText('No guardar');
    });

    test('editar contacto desde modal actualiza los datos', async ({
        page,
    }) => {
        const contact = makeContact({
            name: 'Original',
...
test.describe('Tabs con provide/inject', () => {
    ...
    test('el indicador activo cambia al clickear tabs', async ({ page }) => {
        ...
        await page.locator('[data-tab="favorites"]').click();
        await expect(
            page.locator('[data-tab="favorites"][data-active="true"]'),
        ).toBeVisible();
        await expect(
            page.locator('[data-tab="all"][data-active="true"]'),
        ).not.toBeVisible();
...
test.describe('Accordion y Transitions', () => {
    test.beforeEach(async ({ page }) => {
        await seedContacts(page, [
            makeContact({ notes: 'Nota de prueba', phone: '+34 999 888 777' }),
        ]);
        await waitForApp(page);
    });
...
test.describe('Búsqueda de contactos', () => {
    ...
    test('buscar por nombre filtra la lista', async ({ page }) => {
        ...
        await expect(
            page.locator('[data-contact-list="true"]'),
        ).not.toContainText('Beatriz Sánchez');
    });
    ...
    test('buscar por email filtra la lista', async ({ page }) => {
        ...
        await expect(
            page.locator('[data-contact-list="true"]'),
        ).not.toContainText('Alfonso Rodríguez');
    });
...
test.describe('Responsive', () => {
    ...
    test('el botón Agregar es visible y clickeable en mobile', async ({
        page,
    }) => {
        await waitForApp(page);
    ...
    test('el modal se muestra correctamente en viewports pequeños', async ({
        page,
    }) => {
        await waitForApp(page);
        await page.locator('[data-action="add-contact"]').click();
...
test('screenshot del Contact Manager con datos', async ({ page }) => {
    ...
    await page.screenshot({
        path: 'playwright-report/contact-manager.png',
        fullPage: true,
    });
});

...
describe('Security - Path Validation', () => {
    describe('validatePath', () => {
        ...
        describe('on win32', () => {
            ...
            it('allows a Windows absolute path generated by the CLI itself that resolves inside cwd', async () => {
                vi.resetModules();
                ...
                const winPath = `${process.cwd()}\\versacompile.config.ts`;
                expect(validatePathWin32(winPath)).toBe(true);
            });

            it('still rejects a Windows absolute path outside cwd (real traversal)', async () => {
                vi.resetModules();
                vi.doMock('node:process', async () => {
                    const actual =
                        await vi.importActual<typeof process>('node:process');
                ...
                expect(validatePathWin32('C:\\Windows\\System32')).toBe(
                    false,
                );
            });
        });

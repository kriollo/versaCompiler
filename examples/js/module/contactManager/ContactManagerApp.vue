...
const AsyncStats = defineAsyncComponent({
    ...
    loadingComponent: {
        template: '<div class="text-xs text-gray-400 p-4">Cargando estadísticas...</div>',
    },
    delay: 100,
});
...
const tabs = computed(() => [
    ...
    { id: 'favorites' as const, label: 'Favoritos', count: favoriteContacts.value.length },
    { id: 'recent' as const, label: 'Recientes', count: recentContacts.value.length },
]);
                ...
                <template #default="{ activeTab: tab }">
                    ...
                    <ErrorBoundary>
                        <ContactList
                            ...
                            <template #default="{ contact, onEdit, onDelete, onToggleFavorite, onDragStart }">
                                <ContactCard
                                    :contact="contact"
                                    draggable
                                    ...
                                    <template #actions="{ contact: c }">
                                        <button
                                            ...
                                            :aria-label="c.favorite ? 'Quitar de favoritos' : 'Marcar favorito'"
                                            class="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-lg leading-none"
                                            :class="c.favorite ? 'text-yellow-400' : 'text-gray-300'">
                                            ★
                                        </button>
                                        <button
                                            @click="onEdit()"

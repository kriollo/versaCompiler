<script setup lang="ts">
    ...
    const backoffStrategies = [
        { value: 'exponential', label: 'Exponencial (1s, 2s, 4s...)' },
        { value: 'linear', label: 'Lineal (1s, 2s, 3s...)' },
        { value: 'fixed', label: 'Fijo (siempre igual)' },
    ];
...
<template>
    <div class="space-y-4">
        ...
        <div
            class="p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-lg text-xs text-indigo-700 dark:text-indigo-300">
            <p>
                <span class="font-semibold">Nota:</span>
                Esta herramienta tiene
                <span class="font-semibold">1 salida única</span>
                . El resultado (éxito o error) se devuelve a la IA para que decida cómo continuar.
            </p>
        </div>
            ...
            <template v-if="localConfig.apiRetryEnabled">
                ...
                <div>
                    ...
                    <select
                        ...
                        class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <option v-for="strategy in backoffStrategies" :key="strategy.value" :value="strategy.value">
                            {{ strategy.label }}
                        </option>
                    </select>
                </div>

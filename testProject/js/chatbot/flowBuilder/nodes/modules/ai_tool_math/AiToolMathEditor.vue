<script setup lang="ts">
    import { computed } from 'vue';

    import type { FlowNode, NodeConfig } from '@/dashboard/js/chatbot/flowBuilder/types/flowTypes';

    interface Props {
        modelValue: NodeConfig;
        node: FlowNode;
    }

    const props = defineProps<Props>();
    const emit = defineEmits<{
        'update:modelValue': [val: NodeConfig];
    }>();

    const localConfig = computed<NodeConfig>({
        get: () => props.modelValue,
        set: val => emit('update:modelValue', val),
    });

    const showAB = computed(() =>
        ['add', 'subtract', 'multiply', 'divide', 'power', 'modulo', 'percentage'].includes(
            localConfig.value.mathOperation ?? 'add',
        ),
    );
    const showA = computed(() => ['sqrt', 'abs', 'round'].includes(localConfig.value.mathOperation ?? ''));
    const showExpression = computed(() => localConfig.value.mathOperation === 'evaluate');
    const showDecimals = computed(() => localConfig.value.mathOperation === 'round');
</script>

<template>
    <div class="space-y-4">
        <div class="border-b border-gray-200 dark:border-gray-700 pb-4 mb-4">
            <h4 class="text-sm font-semibold text-purple-600 dark:text-purple-400 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                Definición de Herramienta para IA
            </h4>
            <div class="space-y-3">
                <div>
                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                        Nombre de la herramienta
                    </label>
                    <input
                        v-model="localConfig.toolName"
                        type="text"
                        placeholder="ej: calcular"
                        class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-mono" />
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                        Descripción (instrucción para la IA)
                    </label>
                    <textarea
                        v-model="localConfig.toolDescription"
                        rows="2"
                        placeholder="Usa esta herramienta para cálculos matemáticos precisos..."
                        class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"></textarea>
                </div>
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Operación por defecto</label>
            <select
                v-model="localConfig.mathOperation"
                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                <option value="add">Sumar (a + b)</option>
                <option value="subtract">Restar (a - b)</option>
                <option value="multiply">Multiplicar (a × b)</option>
                <option value="divide">Dividir (a ÷ b)</option>
                <option value="power">Potencia (a ^ b)</option>
                <option value="sqrt">Raíz cuadrada (√a)</option>
                <option value="round">Redondear (round a)</option>
                <option value="percentage">Porcentaje (a% de b)</option>
                <option value="modulo">Módulo (a % b)</option>
                <option value="abs">Valor absoluto (|a|)</option>
                <option value="evaluate">Evaluar expresión</option>
            </select>
        </div>

        <div v-if="showAB" class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Valor A (sugerencia)
                </label>
                <input
                    v-model.number="localConfig.mathValueA"
                    type="number"
                    placeholder="0"
                    class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Valor B (sugerencia)
                </label>
                <input
                    v-model.number="localConfig.mathValueB"
                    type="number"
                    placeholder="0"
                    class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
            </div>
        </div>

        <div v-if="showA">
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Valor A (sugerencia)</label>
            <input
                v-model.number="localConfig.mathValueA"
                type="number"
                placeholder="0"
                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
        </div>

        <div v-if="showDecimals">
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Decimales</label>
            <input
                v-model.number="localConfig.mathDecimals"
                type="number"
                min="0"
                max="10"
                placeholder="2"
                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
        </div>

        <div v-if="showExpression">
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                Expresión matemática (solo +, -, *, /, %)
            </label>
            <input
                v-model="localConfig.mathExpression"
                type="text"
                placeholder="ej: (100 * 1.19) / 2"
                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-mono" />
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Variable para guardar el resultado
            </label>
            <input
                v-model="localConfig.mathOutputVariable"
                type="text"
                placeholder="__math_result"
                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
        </div>

        <div
            class="p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-lg text-xs text-purple-700 dark:text-purple-300">
            El agente puede sobrescribir la operación y los valores en runtime. La configuración aquí es la sugerencia
            por defecto.
        </div>
    </div>
</template>

<script setup lang="ts">
    ...
    import CitasCalendar from '@/dashboard/js/agenda/citas/CitasCalendar.vue';
    import CitasList from '@/dashboard/js/agenda/citas/CitasList.vue';
    import ClientesList from '@/dashboard/js/agenda/clientes/ClientesList.vue';
    import AgendaConfigForm from '@/dashboard/js/agenda/configuracion/AgendaConfigForm.vue';
    import { ShowModalCitaInject, ShowModalClienteInject } from '@/dashboard/js/agenda/InjectKeys';

    // Registrar provides para que los hijos puedan inyectar
    ShowModalClienteInject.provide({ show: false, item: null });
    ShowModalCitaInject.provide({ show: false });

    // ---- Tabs ----
    type TabId = 'citas' | 'clientes' | 'listaCitas';

    const activeTab = ref<TabId>('citas');

    const tabs: { id: TabId; label: string; icon: string }[] = [
        { id: 'citas', label: 'Agenda', icon: 'bi-calendar2-week-fill' },
        { id: 'listaCitas', label: 'Lista de Citas', icon: 'bi-list-ul' },
    ...
    // Ref al calendario para refrescar desde lista de citas
    // Tipado explícito con lo que expone el componente vía defineExpose
    const calendarRef = ref<{ refresh(): void } | null>(null);

    const onCitasListAccion = (data: { accion: string }) => {
        if (data.accion === 'refreshCalendar' && calendarRef.value) {
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-[#050505]">
        ...
        <div class="max-w-screen-xl mx-auto px-6 py-6">
            ...
            <div v-show="activeTab === 'citas'">
                <CitasCalendar ref="calendarRef" />
            </div>

            <!-- TAB: Lista de Citas -->
            <div v-show="activeTab === 'listaCitas'">
                <CitasList @accion="onCitasListAccion" />
            </div>
